...
async function exec() {
  ...
  await prove(
    claimInput,
    path.join(buildDir, "claim_js", "claim.wasm"),
    path.join(buildDir, "claim.zkey"),
    path.join(buildDir, "claim_proof.json"),
    path.join(buildDir, "claim_public.json"),
  );
  ...
  await prove(
    emailSenderInput,
    path.join(buildDir, "email_sender_js", "email_sender.wasm"),
    path.join(buildDir, "email_sender.zkey"),
    path.join(buildDir, "email_sender_proof.json"),
    path.join(buildDir, "email_sender_public.json"),
  );
  log("✓ Proof for email sender circuit generated");

  const announcementInput = await genAnnouncementInput(emailAddr, emailAddrRand);
  await promisify(fs.writeFile)(
    path.join(buildDir, "announcement_input.json"),
    JSON.stringify(announcementInput, null, 2),
  );
  await prove(
    announcementInput,
    path.join(buildDir, "announcement_js", "announcement.wasm"),
    path.join(buildDir, "announcement.zkey"),
    path.join(buildDir, "announcement_proof.json"),
