...
library DecimalUtils {
    ...
    function uintToDecimalString(uint256 amount) public pure returns (string memory) {
        ...
        bytes memory result = new bytes(amountLength > 18 ? amountLength + 1 : 20);
        ...
        uint delta = amountLength > 18 ? 1 : resultLength - amountLength;
        ...
        for (uint8 i = resultLength - 1; i >= 0; i--) {
            ...
            else if (amountLength <= 18 && i < resultLength - amountLength) {
                result[i] = "0";
                actualResultLen++;
            }
